...
import ProfilePictureUpload from '../../professionals/Profile.jsx/ProfilePictureUpload';
import CustomModal from '../../modal/CustomModal';
import EditProfile from '../../professionals/Profile.jsx/EditProfile';
import PostNav from '../MiddleContent/PostNav';
import LikeComment from '../MiddleContent/LikeComment';
...
const fetchUserDetails = async()=>{
    try {
        ...
        const response =await userInstance.get(`/detailsUserPro/${userId}`)
...
const LogedUser = useSelector((state) => (userType === 'users' ? state.ClientAuth.Id : state.proAuth.Id));
...
const setting = async () => {
    try {
      ...
      if (LogedUser) {
        const response = await userInstance.get(`/detailsUserPro/${LogedUser}`);
     ...
     />
        ...
        />)}
            ...
            <Modal isOpen={isOpen} onClose={close}>
              ...
              <ProfilePictureUpload user={user} Type={userType} state={state} setState={setState} changeState={changeState} close={close}/>
            </Modal>
             <p className="font-serif font-semibold">{user?.name}</p>
             {user?.category&&
            <span className="text-sm text-gray-400">{user?.category} -{user?.experiance} year of experiance</span>
             }
            <span className="text-sm text-gray-400">{user?.location} {user?.phone}</span>
          </div>
          <div className="flex justify-center items-center gap-2 my-3">
            <div className="font-semibold text-center mx-4">
              <p className="text-black">  {post
                      ? post.filter((item) => item.proId === user?._id).length
                      : "0"}</p>
              <span className="text-gray-400">Posts</span>
            </div>
            <div className="font-semibold text-center mx-4">
              <p className="text-black"> {user ? user?.following.length : "0"}</p>
         ...
         {LogedUser === user?._id ?(
           ...
           <button 
           onClick={openModal}
           className="bg-pink-500 px-10 py-2 rounded-full text-white shadow-lg">Edit</button>

             <CustomModal isOpen={isModalOpen} onClose={closeModal}>
                  <div className="-mt-16 -mb-20">
                    <EditProfile user={user} Type={userType} changeState={changeState}/>
                  </div>
                </CustomModal>
...
>
            <FollowButton item={user} userId={LogedUser} userType={userType} userDetail={userDetail} token={token} setSuc={setSuc} suce={suce}/>
            </div>
            {/* className="bg-pink-500 px-10 py-2 rounded-full text-white shadow-lg" */}

            <button 
              onClick={()=>goMessage(user?._id,user?.name)}
            className="bg-white border border-gray-500 px-10 py-2 rounded-full shadow-lg">Message</button>
            </>
         )}
          </div>

          <div className="flex ">
            <button className="w-full py-2 border-b-2 border-yellow-400">posts
              <svg
            ...
            <button className="w-full py-2 border-b-2">saved
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="mx-auto h-6 w-6"
                fill="none"
